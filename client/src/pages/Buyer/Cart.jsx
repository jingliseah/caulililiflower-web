    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="card p-16 text-center">
          ...
          <p className="secondary mb-8">Head to the shop and find something you love.</p>
          <Link to="/shop" className="button-primary">Browse Products</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="card p-2 mb-6">
        <div className="divide-y divide-border-primary">
          {items.map((item) => {
            ...
            const key = `${item.product.id}-${item.variation?.id ?? "default"}`;
            return (
              <div key={key} className="flex items-center gap-4 p-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.variation?.name && (
                    <p className="text-body-3 secondary mt-1">{item.variation.name}</p>
                  )}
                  <p className="text-body-3 secondary">RM {unitPrice.toFixed(2)} each</p>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  >−</button>
                  <span className="text-body-2 primary w-6 text-center">{item.quantity}</span>
                  <button
                    onClick={() => updateQuantity(item.product.id, item.variation?.id ?? null, item.quantity + 1)}
                    className="w-8 h-8 rounded-full border border-border-secondary flex items-center justify-center text-body-2 primary hover:bg-surface-secondary transition"
                  >+</button>
                ...
                <div className="text-right shrink-0 w-24">
                  <p className="text-label-1 primary">RM {(unitPrice * item.quantity).toFixed(2)}</p>
                  <button
      ...
      <div className="card p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button onClick={() => navigate("/checkout")} className="flex-1 button-primary">
            Proceed to Checkout
          </button>
          <button onClick={clearCart} className="button-secondary">Clear Cart</button>
        </div>
      </div>
    </div>
  );
